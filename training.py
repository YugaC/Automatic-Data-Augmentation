...
if __name__ == '__main__':
    ...
    fold_index = 1
    for train_idx, val_idx in kf.split(data_dicts):  # Split data into 4 folds
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    if metric > best_metric:
                        ...
                        torch.save(model.state_dict(), os.path.join(root_dir, f"best_metric_model_fold_{fold_index}.pth"))
                        print('organ dice: ', [str(i)[:4] for i in metric_organ])
                        print(f"Saved new best metric model for fold {fold_index}")
                    print(
        ...
        np.save(os.path.join(root_dir, f"train_loss_fold_{fold_index}.npy"), train_loss_values)
        np.save(os.path.join(root_dir, f"val_metrics_fold_{fold_index}.npy"), val_metric_values)


        # Training and Validation Dice Score graph                
        ...
        # Add validation dice score curve (Blue)
        fig.add_trace(go.Scatter(
            x=x_val_epochs, y=y_val_metric, mode='lines+markers', 
            name='Validation Dice Score', 
            line=dict(color='blue', width=3)
        ))

        # Update layout to ensure a clear and clean visualization
        fig.update_layout(
            title="Training Loss and Validation Dice Score",
            xaxis_title="Epoch",
            yaxis_title="Loss/Dice Score",
        ...
        fig.write_image(os.path.join(root_dir, 'train_val_dice_plot.png'))
