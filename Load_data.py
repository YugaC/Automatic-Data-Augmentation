...
def load_data(images_dir, labels_root_dirs):
...
if __name__ == "__main__":
    ...
    label_arrays = []
    for dir in labels_root_dirs:
        label_files = sorted([os.path.join(dir, f) for f in os.listdir(dir) if f.endswith('.nii') or f.endswith('.nii.gz')])
        if not label_files:
            print(f"No label files found in directory {dir}")
        labels = [load_nii_to_array(f) for f in label_files]
        if labels:
            label_arrays.append(np.stack(labels, axis=-1))
        else:
            print(f"Warning: No labels loaded from {dir}. Check directory content or permissions.")
...
if __name__ == "__main__":
    ...
    # Save datasets
    save_dataset(images_train, labels_train, "C:/Users/Yugashree/Automatic-Data-Augmentations(data)/new_data/train")
    save_dataset(images_test, labels_test, "C:/Users/Yugashree/Automatic-Data-Augmentations()/new_data/valid")
=======
# data_loader.py
import nibabel as nib
import numpy as np
...
def prepare_datasets(data_dir):
    ...
    return train, test
>>>>>>> 49e653b0ef6ed56b5bcd400171f483de63721501
